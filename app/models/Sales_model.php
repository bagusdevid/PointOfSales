...
class Sales_model extends CI_Model
{
    ...
    public function updatePengiriman($id, $status, $kurir) {
        $date = date('Y-m-d H:i:s');
        if ($this->db->update('sales', array('resi' => $status, 'kurir' => $kurir, 'tgl_input' => $date), array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    public function addResi($data = array()) {
        if ($this->db->insert_batch('sales', $data)) {
            return true;
        }
        return false;
    }
