...
class Escpos
{
    ...
    function print_receipt($store, $sale, $items, $payments, $created_by, $open_drawer = false, $bill = false) {
        ...
        $this->printer->feed();
        $this->printer->setJustification(Printer::JUSTIFY_LEFT);
        $this->printer->text(lang('date') . ': '.$this->izi->hrld($sale->date) . "\n");
        ...
        foreach ($items as $item) {
            $this->printer->text('#'.$r.' '.$this->product_name(addslashes($item->product_name)) . "\n");
            $this->printer->text($this->printLine('   '.$this->izi->formatQuantity($item->quantity) . " x " . $this->izi->formatMoney($item->net_unit_price + ($item->item_tax / $item->quantity)) . ":  " . $this->izi->formatMoney($item->subtotal)) . "\n");
            $r++;
        }

        $this->printer->text($this->drawLine());
        $this->printer->text($this->printLine(lang("total").":".$this->izi->formatMoney($sale->total + $sale->product_tax)) . "\n");
        if ($sale->order_tax != 0) {
            $this->printer->text($this->printLine(lang("order_tax").":".$this->izi->formatMoney($sale->order_tax)) . "\n");
        }
        if ($sale->total_discount != 0) {
            $this->printer->text($this->printLine(lang("order_discount").":".$this->izi->formatMoney($sale->total_discount)) . "\n");
        }
        if ($this->Settings->rounding) {
            $round_total = $this->izi->roundNumber($sale->grand_total, $this->Settings->rounding);
            $rounding = $this->izi->formatMoney($round_total - $sale->grand_total);
            $this->printer->text($this->printLine(lang("rounding").":".$this->izi->formatMoney($rounding)) . "\n");
            $this->printer->text($this->printLine(lang("grand_total").":".$this->izi->formatMoney($sale->grand_total + $rounding)) . "\n");
        }
        else {
            $round_total = $sale->grand_total;
            $this->printer->text($this->printLine(lang("grand_total").":".$this->izi->formatMoney($sale->grand_total)) . "\n");
        }
        if ($sale->paid < $round_total && !$bill) {
            $this->printer->text($this->printLine(lang("paid_amount").":".$this->izi->formatMoney($sale->paid)) . "\n");
            $this->printer->text($this->printLine(lang("due_amount").":".$this->izi->formatMoney($sale->grand_total - $sale->paid)) . "\n");
        }
            ...
            if($payments) {
                foreach($payments as $payment) {
                    if ($payment->paid_by == 'cash' && $payment->pos_paid) {
                        ...
                        $this->printer->text($this->printLine(lang("amount").":".$this->izi->formatMoney($payment->pos_paid)) . "\n");
                        $this->printer->text($this->printLine(lang("change").":".($payment->pos_balance > 0 ? $this->izi->formatMoney($payment->pos_balance) : 0)) . "\n");
                    }
                    ...
                    elseif ($payment->paid_by == 'other' && $payment->amount) {
                        $this->printer->text($this->printLine(lang("paid_by").":".lang($payment->paid_by)) . "\n");
                        $this->printer->text($this->printLine(lang("amount").":".$this->izi->formatMoney($payment->amount)) . "\n");
    ...
    function print_order($store, $sale, $items, $created_by) {
        ...
        $this->printer->text('T: '.$this->izi->hrld($sale->date) . "\n");
        $this->printer->feed();
        ...
        foreach ($items as $item) {
            ...
            $this->printer->text($this->printLine('#'.$r.' '.$this->product_name(addslashes($item->product_name)).' ('.$item->product_code.') : [ ' .
                        ($item->quantity > 0 ? $this->izi->formatQuantity($item->quantity) : 'xxxx')." ]"). "\n");
