...
class Pos extends MY_Controller {
    ...
    function index($sid = NULL, $eid = NULL) {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $note = $this->izi->clear_tags($this->input->post('spos_note'));
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $real_unit_price = $this->izi->formatDecimal($_POST['real_unit_price'][$r]);
                ...
                if (isset($item_id) && isset($real_unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_discount)) {
                        ...
                        if ($dpos !== false) {
                            ...
                            $pr_discount = (($this->izi->formatDecimal($unit_price)) * (Float)($pds[0])) / 100;
                        } else {
                            $pr_discount = $this->izi->formatDecimal($discount);
                        }
                    }
                    $unit_price = $this->izi->formatDecimal($unit_price - $pr_discount);
                    $item_net_price = $unit_price;
                    $pr_item_discount = $this->izi->formatDecimal($pr_discount * $item_quantity);
                    ...
                    $pr_item_tax = 0; $item_tax = 0; $tax = "";
                        if (isset($product_details->tax) && $product_details->tax != 0) {
                            ...
                            if ($product_details && $product_details->tax_method == 1) {
                                $item_tax = $this->izi->formatDecimal((($unit_price) * $product_details->tax) / 100);
                                $tax = $product_details->tax . "%";
                            } else {
                                $item_tax = $this->izi->formatDecimal((($unit_price) * $product_details->tax) / (100 + $product_details->tax));
                                $tax = $product_details->tax . "%";
                                $item_net_price -= $item_tax;
                            }

                            $pr_item_tax = $this->izi->formatDecimal($item_tax * $item_quantity);
            ...
            if ($this->input->post('order_discount')) {
                ...
                if ($opos !== false) {
                    ...
                    $order_discount = $this->izi->formatDecimal((($total + $product_tax) * (Float)($ods[0])) / 100);
                } else {
                    $order_discount = $this->izi->formatDecimal($order_discount_id);
                }
            } else {
                $order_discount_id = NULL;
            }
            $total_discount = $this->izi->formatDecimal($order_discount + $product_discount);
            ...
            if($this->input->post('order_tax')) {
                ...
                if ($opos !== false) {
                    ...
                    $order_tax = $this->izi->formatDecimal((($total + $product_tax - $order_discount) * (Float)($ots[0])) / 100);
                } else {
                    $order_tax = $this->izi->formatDecimal($order_tax_id);
                }
            ...
            $total_tax = $this->izi->formatDecimal($product_tax + $order_tax);
            $grand_total = $this->izi->formatDecimal($this->izi->formatDecimal($total) + $total_tax - $order_discount);
            ...
            $round_total = $this->izi->roundNumber($grand_total, $this->Settings->rounding);
            $rounding = $this->izi->formatDecimal($round_total - $grand_total);
            ...
            $data = array('date' => $date,
                ...
                'total' => $this->izi->formatDecimal($total),
                'product_discount' => $this->izi->formatDecimal($product_discount),
                'order_discount_id' => $order_discount_id,
                'order_discount' => $order_discount,
                'total_discount' => $total_discount,
                'product_tax' => $this->izi->formatDecimal($product_tax),
            ...
            if (!$eid && !$suspend && $paid) {
                ...
                $amount = $this->izi->formatDecimal($paid > $grand_total ? ($paid - $this->input->post('balance_amount')) : $paid);
                $payment = array(
                    'date' => $date,
                    'amount' => $amount,
                    'customer_id' => $customer_id,
                    ...
                    'pos_paid' => $this->izi->formatDecimal($this->input->post('amount')),
                    'pos_balance' => $this->izi->formatDecimal($this->input->post('balance_amount'))
                    );
                $data['paid'] = $amount;
            ...
            // $this->izi->print_arrays($data, $products, $payment);
        }
        ...
        else
        {
            ...
            if ($saleid = $this->input->get('print', true)) {
                if ($inv = $this->pos_model->getSaleByID($saleid)) {
                    ...
                    $this->izi->view_rights($inv->created_by, false, 'pos');
    ...
    function close_register($user_id = NULL) {
        ...
        if ($this->form_validation->run() == true && $this->pos_model->closeRegister($rid, $user_id, $data)) {
            ...
            $this->data['users'] = $this->izi->getUsers($user_id);
    ...
    function view($sale_id = NULL, $noprint = NULL) {
        ...
        $this->izi->view_rights($inv->created_by);
        $this->load->helper('text');
	...
	function kirim($sale_id = NULL, $noprint = NULL) {
        if($this->input->get('id')){ $sale_id = $this->input->get('id'); }
        $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
        $this->data['message'] = $this->session->flashdata('message');
        $inv = $this->pos_model->getSaleByID($sale_id);
        ...
        $this->data['store'] = $this->site->getStoreByID($inv->store_id);
        $this->data['page_title'] = lang("invoice");
        $this->load->view($this->theme.'pos/kirim', $this->data);
    ...
    function email_receipt($sale_id = NULL, $to = NULL) {
        ...
        try {
            if ($this->izi->send_email($to, $subject, $message)) {
    ...
    function print_register($re = NULL) {
        ...
        if ($this->session->userdata('register_id')) {
            ...
            $total_cash -= ($expenses->total ? $expenses->total : 0);
            $info = array(
                (object) array('label' => lang('opened_at'), 'value' => $this->izi->hrld($register->date)),
                ...
                (object) array('label' => lang('printed_at'),  'value' => $this->izi->hrld(date('Y-m-d H:i:s')))
                );

            $reg_totals = array(
                (object) array('label' => lang('cash_sale'), 'value' => $this->izi->formatMoney($cashsales->paid ? $cashsales->paid : '0.00') . ' (' . $this->izi->formatMoney($cashsales->total ? $cashsales->total : '0.00') . ')'),
                (object) array('label' => lang('ch_sale'), 'value' => $this->izi->formatMoney($chsales->paid ? $chsales->paid : '0.00') . ' (' . $this->izi->formatMoney($chsales->total ? $chsales->total : '0.00') . ')'),
                (object) array('label' => lang('gc_sale'),  'value' => $this->izi->formatMoney($gcsales->paid ? $gcsales->paid : '0.00') . ' (' . $this->izi->formatMoney($gcsales->total ? $gcsales->total : '0.00') . ')'),
                (object) array('label' => lang('cc_sale'),  'value' => $this->izi->formatMoney($ccsales->paid ? $ccsales->paid : '0.00') . ' (' . $this->izi->formatMoney($ccsales->total ? $ccsales->total : '0.00') . ')'),
                (object) array('label' => lang('stripe'),  'value' => $this->izi->formatMoney($stripesales->paid ? $stripesales->paid : '0.00') . ' (' . $this->izi->formatMoney($stripesales->total ? $stripesales->total : '0.00') . ')'),
                (object) array('label' => lang('other_sale'),  'value' => $this->izi->formatMoney($other_sales->paid ? $other_sales->paid : '0.00') . ' (' . $this->izi->formatMoney($other_sales->total ? $other_sales->total : '0.00') . ')'),
                (object) array('label' => 'line',  'value' => ''),
                (object) array('label' => lang('total_sales'),  'value' => $this->izi->formatMoney($totalsales->paid ? $totalsales->paid : '0.00') . ' (' . $this->izi->formatMoney($totalsales->total ? $totalsales->total : '0.00') . ')'),
                (object) array('label' => lang('cash_in_hand'),  'value' => $this->izi->formatMoney($register->cash_in_hand)),
                (object) array('label' => lang('expenses'),  'value' => $this->izi->formatMoney($expenses->total ? $expenses->total : '0.00')),
                (object) array('label' => 'line',  'value' => ''),
                (object) array('label' => lang('total_cash'),  'value' => $this->izi->formatMoney($total_cash))
                );
