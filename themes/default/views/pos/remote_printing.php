...
if ($Settings->remote_printing == 2) {
    ...
    <script type="text/javascript">
        function receiptData() {
            ...
            receipt.info = "";
            receipt.info += "<?= lang("date") . ": " . $this->izi->hrld($inv->date); ?>" + "\n";
            ...
            receipt.items += "<?= printLine("   ".$this->izi->formatQuantity($row->quantity)." x ".$this->izi->formatMoney($row->net_unit_price+($row->item_tax/$row->quantity)) . ":  ". $this->izi->formatMoney($row->subtotal), $printer->char_per_line, ' ') . ""; ?>" + "\n";
            receipt.items += "<?php $r++; endforeach; ?>";

            receipt.totals = "";
            receipt.totals += "<?= printLine(lang("total") . ": " . $this->izi->formatMoney($inv->total+$inv->product_tax), $printer->char_per_line); ?>" + "\n";
            <?php
            if ($inv->order_tax != 0) { ?>
                receipt.totals += "<?= printLine(lang("tax") . ": " . $this->izi->formatMoney($inv->order_tax), $printer->char_per_line); ?>" + "\n";
                <?php
            }
            if ($inv->total_discount != 0) { ?>
                receipt.totals += "<?= printLine(lang("discount") . ": " . $this->izi->formatMoney($inv->total_discount), $printer->char_per_line); ?>" + "\n";
                <?php
            }
            if ($Settings->rounding) { ?>
                receipt.totals += "<?= printLine(lang("rounding") . ": " . $this->izi->formatMoney($rounding), $printer->char_per_line); ?>" + "\n";
                receipt.totals += "<?= printLine(lang("grand_total") . ": " . $this->izi->formatMoney($inv->grand_total + $rounding), $printer->char_per_line); ?>" + "\n";
                <?php
            } else { ?>
                receipt.totals += "<?= printLine(lang("grand_total") . ": " . $this->izi->formatMoney($inv->grand_total), $printer->char_per_line); ?>" + "\n";
                <?php
            }
            if ($inv->paid < $inv->grand_total) { ?>
                receipt.totals += "<?= printLine(lang("paid_amount") . ": " . $this->izi->formatMoney($inv->paid), $printer->char_per_line); ?>" + "\n";
                receipt.totals += "<?= printLine(lang("due_amount") . ": " . $this->izi->formatMoney($inv->grand_total-$inv->paid), $printer->char_per_line); ?>" + "\n\n";
                <?php
            } ?>
            ...
            if ($payments) {
                ...
                foreach($payments as $payment) {
                    if ($payment->paid_by == 'cash' && $payment->pos_paid) { ?>
                        ...
                        receipt.payments += "<?= printLine(lang("amount") . ": " . $this->izi->formatMoney($payment->pos_paid), $printer->char_per_line); ?>" + "\n";
                        receipt.payments += "<?= printLine(lang("change") . ": " . ($payment->pos_balance > 0 ? $this->izi->formatMoney($payment->pos_balance) : 0), $printer->char_per_line); ?>" + "\n";
                        <?php
                        ...
                        receipt.payments += "<?= printLine(lang("paid_by") . ": " . lang($payment->paid_by), $printer->char_per_line); ?>" + "\n";
                        receipt.payments += "<?= printLine(lang("amount") . ": " . $this->izi->formatMoney($payment->amount), $printer->char_per_line); ?>" + "\n";
            ...
            <?php
            if ($inv->note) { ?>
                receipt.footer += "<?= printText(strip_tags(preg_replace('/\s+/',' ', $this->izi->decode_html($inv->note))), $printer->char_per_line); ?>" + "\r\n\r\n";
                <?php
            } ?>
